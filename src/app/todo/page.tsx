...
export default function TodoPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/processes")
      ...
      .then((d) => {
        if (Array.isArray(d))
          setProcesses(
            d.map((p: { id: string; subject: string }) => ({
              id: p.id,
              subject: p.subject,
            })),
          );
    ...
    const res = await fetch("/api/tasks", {
      ...
      body: JSON.stringify({
        ...
        deadline: newDeadline
          ? new Date(newDeadline).toLocaleDateString("pt-BR")
          : "",
    ...
    const res = await fetch(`/api/tasks/${editTask.id}`, {
      ...
      body: JSON.stringify({
        ...
        deadline: editDeadline
          ? new Date(editDeadline).toLocaleDateString("pt-BR")
          : "",
  ...
  const filtered = tasks.filter((t) => {
    ...
    if (filterResponsible !== "all" && t.responsible.id !== filterResponsible)
      return false;
    ...
    <div className="py-6">
          ...
          <DialogContent className="max-w-sm">
            ...
            <div className="flex flex-col gap-4 mt-2">
              ...
              <div>
                ...
                <Select
                  value={newPriority}
                  onValueChange={(v) => setNewPriority(v as "Alta" | "Normal")}
                >
      ...
      <div className="flex flex-col gap-2 my-4">
        ...
        {filtered.map((task) => (
          <div
            key={task.id}
            className="flex items-start gap-3 rounded-lg border px-4 py-3 bg-card"
          >
            <Checkbox
              checked={task.done}
            ...
            <div className="flex-1 min-w-0">
              <p
                className={`text-sm font-medium leading-snug ${task.done ? "line-through text-muted-foreground" : ""}`}
              >
                {task.text}
              </p>
                  ...
                  <div className="flex flex-col gap-4 mt-2">
                    ...
                    <div>
                      ...
                      <Select
                        value={editPriority}
                        onValueChange={(v) =>
                          setEditPriority(v as "Alta" | "Normal")
                        }
                      >
                        <SelectTrigger className="mt-1">
                          <SelectValue />
